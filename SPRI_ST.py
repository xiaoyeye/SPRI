...
def compute_genes_pvalue(file_path,method,num_perm,num_jobs):
    # preprocess the dataset
    pd_file = pd.read_csv(file_path, sep='\t')
...
if __name__ == '__main__': 
    ...
    if dataname == 'MOB12':
        file_path = 'raw_data/Rep12_MOB_count_matrix-1.tsv' # MOB12


    ##################### read data filter out spots and genes
    pd_file = pd.read_csv(file_path, sep='\t')
    cell_x,cell_y,pd_file = filter_df(pd_file)
    ...
    ############################## if perform permutation on genes:p-val
    p_values, scores, genes = compute_genes_pvalue(file_path=file_path, method='TIC',num_perm = 10000,num_jobs=40)
