...
def calculate_mic_gene_both(cell_x,cell_y,gene_count):
    ...
    x = cell_x[index]
    y = cell_y[index]
    for conut_index in range(len(count)):
        x_count_list.extend([x[conut_index]]*count[conut_index])
        y_count_list.extend([y[conut_index]]*count[conut_index])
...
def plot_bar(gene,count_list,gene_index,save_path,png_type):
    ...
    plt.savefig(save_path + str(gene_index)+'_'+gene + '_'+png_type+'.png')
    plt.close()
...
def compute_genes_pvalue(file_path,top_genes,method,num_perm,num_jobs):
    ...
    BG_filepath = 'processed_data/'+ dataname + '/' + dataname + '_background_spot.csv'
    ...
    for gene in genes:
        ...
        # plot_bar(gene,gene_count_noise,gene_index,'Fig_addBG/','BG')
        # plot_bar(gene,gene_count_plot + gene_count_noise,gene_index,'Fig_addBG/','final')
        ...
        # score, permutation_scores, pvalue = permutation_test(cell_x,cell_y,gene,gene_count,scoring=method,n_permutations=num_perm,
...
def permutation_test(cell_x,cell_y,gene,gene_count,scoring="TIC",n_permutations=1000,
    ...
    pvalue = (np.sum(permutation_scores >= score_true)) / float((n_permutations))
...
if __name__ == '__main__': 
    # load raw data
    dataname = '151507'
    file_path = 'processed_data/' + dataname + '/' + dataname + '_count_matrix.csv' # 151673
    ...
    # thred = 0.1*len(df)
    # thred = int(thred)
    # top_genes = genes[0:thred] # 3353
    top_genes = genes

    p_values, scores, genes = compute_genes_pvalue(file_path=file_path,top_genes = top_genes, method='TIC',num_perm = 1000,num_jobs=40)
