...
def compute_genes_MIC(infoPath,spotsPath):
    # preprocess the dataset
    pd_spots = pd.read_csv(spotsPath)
    pd_counts = pd.read_csv(infoPath,sep='\t')
    pd_file = pd_counts[pd_counts['Unnamed: 0'].isin(pd_spots['Unnamed: 0'])]
...
if __name__ == '__main__': 
    ...
    dataname = 'BC2020'
    if dataname == 'BC2020':
        infoPath = 'raw_data/BC23209_C1_stdata.tsv'
        spotsPath = 'raw_data/spots_BC23209_C1.csv'

    ############################## compute TIC rank 
    MIC_list, TIC_list, genes = compute_genes_MIC(infoPath=infoPath,spotsPath = spotsPath)
